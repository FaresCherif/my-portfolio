      ...
      <div className="flex flex-col gap-12">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {["PHP", "JavaScript", "Java", "C++", "Python", "SQL"].map((tech) => (
              <div key={tech} className="border border-gray-700 rounded-lg px-4 py-3 text-center text-gray-300 hover:border-blue-500 transition">
                {tech}
              </div>
            ))}
          </div>
        ...
        {/* Formation */}
        <div>
          <h3 className="text-blue-500 font-semibold uppercase text-sm tracking-widest mb-6">Formation et Expérience</h3>
          <div className="flex flex-col gap-4">
            {[
              { year: "2022 - Aujourd'hui", title: "Developpeur", school: "Einden" },
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {["Tennis", "Course à pied", "Échecs", "Littérature", "Voyages"].map((item) => (
              <span key={item} className="bg-gray-800 text-gray-300 px-4 py-2 rounded-full text-sm">
                {item}
              </span>
            ))}
          </div>
